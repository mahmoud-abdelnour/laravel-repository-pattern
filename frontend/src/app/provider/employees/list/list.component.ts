...
export class ListComponent implements OnInit {
  ...
  openDialog(mode = FormMode.Create, data = null) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) this.updateTables()
    });
  }

  suggestCourses(data = null) {
    const dialogRef = this.matDialog.open(this.dialog, {
      data:{
        data,
      }
  ...
  popDelete(rowData) {
    ...
    dialog.componentInstance.header = 'DeleteEmployeeTitle';
    dialog.componentInstance.body = 'DeleteEmployeeMSG';
  }
  ...
  takeAction(action, rowData) {
    let dialog = this.matDialog.open(DeleteDialogComponent);
    dialog.componentInstance.header = action +'EmployeeTitle';
    dialog.componentInstance.body = action +'EmployeeMSG';
    dialog.componentInstance.confirmBtn = action +'Btn';
    
    dialog.afterClosed().subscribe( act => {
      if(act) {
        (
          action == 'Ban' ?
            this.apiService.put(Endpoints.provider.employees + '/' + rowData.id + '/ban') :
            this.apiService.put(Endpoints.provider.employees + '/' + rowData.id + '/admit')
          ).subscribe(r => {
          this.themeService.openSnackBar(action + 'SuccessMSG', null);
          this.updateTables();
        })
      }
    })
  ...
  openEmployeeDialog(data, selectedIndex = 0) {
    ...
    dialogRef.componentInstance.title = 'EmployeeDetails';
  }
